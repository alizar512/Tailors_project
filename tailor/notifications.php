...
try {
    ...
    $stmt = $pdo->prepare("SELECT * FROM notifications WHERE recipient_type = 'tailor' AND recipient_id = ? ORDER BY created_at DESC");
    $stmt->execute([$tailor_id]);
    $notifications = $stmt->fetchAll(PDO::FETCH_ASSOC);
    silah_mark_notifications_read($pdo, 'tailor', $tailor_id);
} catch (Exception $e) {
    $notifications = [];
}
...
<div class="glass-card overflow-hidden max-w-4xl mx-auto">
    <div class="px-8 py-6 border-b border-gray-100 flex items-center justify-between bg-white/50">
        <div>
            <h3 class="text-xl font-black text-primary mb-1">Notifications</h3>
            <p class="text-xs text-gray-500 font-medium">Updates about chats, orders, and payments</p>
        </div>
    </div>
            ...
            <div class="space-y-4">
                <?php foreach ($notifications as $n): ?>
                    <?php
                        ...
                        $href = isset($n['link']) && trim((string)$n['link']) !== '' ? (string)$n['link'] : '';
